##3 years
dvsensitivity <- function(dvs,day, colour, legends){
  for (i in 1:length(dvs)){ 
    ...
    param$DV <- dvs[i] # Diffusivity (m^2/d)
    ...
    if (i==1){
      par(mfrow=c(1,1))
      par(mar = c(5, 4, 4, 2))
      plot(Pplot[day,], param$z, ylim = rev(range(param$z)), type="l", lwd=3, col = colfunc[i], xlim=c(0,5),
